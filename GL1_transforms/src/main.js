...
async function main() {
	...
	*/
	...
	function update_cam_transform(frame_info) {
		...
		* r = cam_distance_base * cam_distance_factor
		...
		*/
		...
		let newcamera = [-r * Math.cos(cam_angle_z) * Math.cos(cam_angle_y) , r * Math.cos(cam_angle_y) * Math.sin(cam_angle_z), r * Math.sin(- cam_angle_y)]
		let rotatup = [0,0,1];

		if (((Math.abs(-cam_angle_y) % (2*Math.PI)) >= Math.PI / 2) && ((Math.abs(-cam_angle_y)  % (2*Math.PI)) <=( 3* Math.PI) / 2)){ //|| (Math.abs(-cam_angle_y) = ((3*Math.PI) / 2))) {  
			rotatup = [0, 0, -1];
		}
		
		const look_at = mat4.lookAt(mat4.create(), 
			//new matrix with the new localisation of camera [-5, 0, 0], // camera position in world coord
			newcamera,
