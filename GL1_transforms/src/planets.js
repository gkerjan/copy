...
export class SysOrbitalMovement {
	...
	calculate_model_matrix(actor, sim_time, actors_by_name) {
		...
		*/
		...
		const actor_scaling = mat4.fromScaling(mat4.create(), [actor.size, actor.size, actor.size]);
		const actor_rotation = mat4.fromZRotation(mat4.create(), sim_time * actor.rotation_speed);

		if(actor.orbit != null) {
			// Parent's translation
			const parent = actors_by_name[actor.orbit]
			const parent_mat = mat4.getTranslation([0, 0, 0], parent.mat_model_to_world)
			const parent_translation_v = mat4.fromTranslation(mat4.create(), parent_mat)
			...
			const rotation_angle = mat4.fromZRotation(mat4.create(), angle);
            
			const radius_transla = mat4.fromTranslation(mat4.create(), [radius,0,0])


			mat4.multiply(M_orbit, M_orbit, parent_translation_v);
            mat4.multiply(M_orbit, M_orbit, rotation_angle);
			mat4.multiply(M_orbit, M_orbit, radius_transla);


        }
        
        // Apply self-rotation and scaling
        mat4.multiply(M_orbit, M_orbit, actor_rotation);
        mat4.multiply(M_orbit, M_orbit, actor_scaling);
        
        // Store the final transformation matrix
        actor.mat_model_to_world = M_orbit;
