...
async function main() {
	...
	*/
	...
	regl.frame((frame) => {
		...
		// Matrices to use
		const M_translation = mat4.fromTranslation(mat4.create(), [0.5, 0, 0]);
		const M_rotation = mat4.fromZRotation(mat4.create(), 30 * Math.PI / 180 * sim_time);
		const M_RT = mat4.multiply(mat4.create(), M_translation, M_rotation);
		...
		#TODO GL1.1.2.2
			...
			let M_rotation = mat4.fromZRotation(mat4.create(), time * 30);
		...
		*/
		...
		draw_triangle_with_transform({
			mat_transform: mat4.multiply(mat4.create(), M_rotation, M_translation),
			color: color_green,
		});
